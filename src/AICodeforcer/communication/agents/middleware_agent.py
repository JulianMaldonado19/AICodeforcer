"""Middleware agent for communication problems."""
...
MIDDLEWARE_PROMPT = """<role>
You are a middleware specialist for communication problems.
Your task is to transform Alice's output into Bob's input.
</role>

<input-format>
Read from stdin: original_input + "===SEPARATOR===" + alice_output
</input-format>

<output-format>
Output Bob's input to stdout.
...
def main():
    data = sys.stdin.read()
    original_input, alice_output = data.split("===SEPARATOR===")
    # Transform logic...
    print(bob_input)
...
class MiddlewareAgent:
    """Generates middleware for communication problems."""
    ...
    def generate(self, problem_text: str) -> str | None:
        ...
        config = types.GenerateContentConfig(
            system_instruction=MIDDLEWARE_PROMPT,
            temperature=1.0,
            max_output_tokens=_max_output_tokens,
            thinking_config=types.ThinkingConfig(thinking_level="high"),
        )

        prompt = f"""请为以下通讯题生成中间件：

{problem_text}

中间件负责将 Alice 的输出转换为 Bob 的输入。
使用 "===SEPARATOR===" 作为分隔符。"""
        ...
        self._api_logger.init(prefix="comm_middleware", model=self.model)
