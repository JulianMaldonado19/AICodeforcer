...
<available-tools>
  ...
  <tool name="interactive_stress_test">
    ...
    <description>Interactive stress test verification</description>
...
<completion-flag>
  <strict-requirements>
    ...
    <requirement>Must call `interactive_stress_test` for stress test verification</requirement>
...
TOOL_DECLARATIONS = [
    types.Tool(function_declarations=[
        ...
        types.FunctionDeclaration(
            ...
            description="交互对拍验证工具：运行交互测试验证你的代码。你只需提供 solution_code，评测机和数据生成器已由系统生成。代码必须完整自包含，所有 print 必须使用 flush=True。",
            parameters=types.Schema(
                type=types.Type.OBJECT,
                properties={
...
所有 print 语句必须使用 flush=True。"""
        ...
        for turn in range(max_attempts):
            ...
            if function_calls:
                ...
                if stress_test_passed and verified_code:
                    print(f"\n[程序化校验] 对拍已通过 {INTERACTIVE_STRESS_TEST_NUM} 次测试，直接返回验证过的代码")
                    self._log(f"[程序化校验] 对拍已通过 {INTERACTIVE_STRESS_TEST_NUM} 次测试，直接返回验证过的代码")
                    self._contents = contents
                    self._config = config
                    self._last_verified_code = verified_code
                    self._last_code = verified_code
