...
<available-tools>
  ...
  <tool name="stress_test">
    ...
    <description>Stress test verification</description>
...
<core-principles title="Must Not Be Violated">
  ...
  <principle>Greedy / Binary Search / Math problems: No verification (proof or experiments) = Not trustworthy</principle>
  ...
  <principle>When brute force is infeasible, prioritize stronger proofs and targeted adversarial tests over blind confidence</principle>
</core-principles>
...
<problem-solving-process title="Mandatory - Must Be Demonstrated">
  ...
  <step name="Complexity Target Setting">
    <task>Infer the maximum feasible time complexity from constraints</task>
    <task>Set a target (e.g., O(n), O(n log n), O(n sqrt n), O(n^2)) and memory budget</task>
    <task>Rule out approaches that violate the target</task>
  </step>
  ...
  <step name="Boundary Checklist">
    <task>List edge cases: min/max sizes, empty structures, duplicates, negative values, sorted/reverse, all-equal, single-element</task>
    <task>Confirm behavior for multiple test cases and strict output format</task>
  </step>

  <step name="Layered Verification Strategy" importance="core-capability">
    <requirement>Use tools to complete at least one of the following, prioritized by feasibility:</requirement>
    <task>Property validation (monotonicity / invariant / optimal substructure)</task>
    <task>Small-scale enumeration to detect patterns or counterexamples</task>
    <task>Randomized testing against brute force (if feasible)</task>
    <task>Stress test verification</task>
    <note>If brute force is infeasible, strengthen proof and perform targeted adversarial tests</note>
  </step>
  ...
  <step name="Correctness Proof Key Points">
    ...
    <proof-type name="Greedy">Exchange argument + invariant + local-to-global optimality link</proof-type>
    <proof-type name="DP">State meaning + transition correctness + base cases + order validity + optimal substructure</proof-type>
    <proof-type name="Binary Search">Monotonic predicate + boundary initialization + termination correctness</proof-type>
    <proof-type name="Math">From enumeration → induction / invariant → general conclusion</proof-type>
  </step>
...
<algorithm-templates title="Template Mapping Hints">
  <template>Prefix sum / difference array for range sums and range updates</template>
  <template>Two pointers / sliding window for contiguous subarray constraints</template>
  <template>Binary search on answer for monotone feasibility</template>
  <template>Greedy with exchange argument for interval scheduling / ordering</template>
  <template>DP: knapsack, LIS, interval DP, tree DP, digit DP, bitmask DP</template>
  <template>Graph: BFS/DFS, topological order, shortest paths (Dijkstra/0-1 BFS), MST</template>
  <template>Disjoint Set Union for connectivity / component merging</template>
  <template>Segment tree / Fenwick for range query and point update</template>
  <template>String: KMP/Z for pattern matching, hashing for substring checks</template>
  <template>Math: number theory (gcd, sieve), combinatorics (nCr mod, factorials)</template>
  <template>Monotonic stack / deque for next greater element, sliding window max</template>
  <template>LCA / binary lifting for tree ancestor queries</template>
  <template>Meet-in-the-middle for medium n with exponential subparts</template>
</algorithm-templates>

<python-performance title="Python Performance Tips">
  <technique>Prefer `sys.stdin.buffer.read()` with fast integer parsing for very large input</technique>
  <technique>Use `sys.stdout.write()` or accumulate outputs in list then `'\\n'.join()`</technique>
  <technique>Bind hot functions and globals to local variables inside loops</technique>
  <technique>Use iterative DFS/BFS to avoid recursion depth issues; set recursion limit if needed</technique>
  <technique>Preallocate lists where possible; avoid repeated list concatenation in loops</technique>
  <technique>Use `heapq`, `bisect`, `array`, `deque` for efficient core operations</technique>
  <technique>Use `@lru_cache` for memoized recursion with careful state size control</technique>
...
<error-diagnosis title="Failure Localization Process">
  <step>If stress test fails, capture the smallest counterexample</step>
  <step>Compare outputs of optimized vs brute force to identify the first divergence</step>
  <step>Instrument the solution with assertions or logging for the failing case only</step>
  <step>Identify the violated invariant or incorrect transition / boundary handling</step>
  <step>Fix, then re-run stress test</step>
</error-diagnosis>

<completion-flag>
  <strict-requirements>
    <requirement>Must call `run_python_code` tool to test all examples given in the problem, and all must pass</requirement>
    <requirement>Must call `stress_test` tool for stress test verification</requirement>
...
TOOL_DECLARATIONS = [
    types.Tool(function_declarations=[
        ...
        types.FunctionDeclaration(
            name="stress_test",
            description="对拍验证工具：比较你的优化算法和系统预生成的暴力算法的输出。你只需提供 solution_code，暴力算法和数据生成器已由系统在独立会话中生成。代码必须完整自包含，从 stdin 读取、向 stdout 输出。",
            parameters=types.Schema(
                type=types.Type.OBJECT,
                properties={
...
记住：必须调用 run_python_code 测试样例，必须调用 stress_test 进行对拍验证。"""
        ...
        for turn in range(max_attempts):
            ...
            if function_calls:
                ...
                if stress_test_passed and verified_code:
                    print(f"\n[程序化校验] 对拍已通过 {STRESS_TEST_NUM} 次测试，直接返回验证过的代码")
                    self._log(f"[程序化校验] 对拍已通过 {STRESS_TEST_NUM} 次测试，直接返回验证过的代码")
                    self._contents = contents
                    self._config = config
                    self._last_verified_code = verified_code
                    self._last_code = verified_code
